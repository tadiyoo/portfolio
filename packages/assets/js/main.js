...
// Making a webpage completely uncopiable, uninspectable, and disabling right-click 
...
// Disable right-click
  ...
  document.addEventListener('keydown', function(event) {
      // Block Ctrl+C, Ctrl+V, Ctrl+S, Ctrl+U, Ctrl+P, Ctrl+A
      if (event.ctrlKey && ['c', 'v', 's', 'u', 'p', 'a'].includes(event.key.toLowerCase())) {
          event.preventDefault();
      }
      // Block F12, Ctrl+Shift+I, Ctrl+Shift+J
      if (event.key === 'F12' || (event.ctrlKey && event.shiftKey && ['i', 'j'].includes(event.key.toLowerCase()))) {
          event.preventDefault();
      }
      // Block Print Screen
      ...
      // Attempt to block Win+Shift+S
      if (event.metaKey && event.shiftKey && event.key.toLowerCase() === 's') {
          event.preventDefault();
          document.body.style.visibility = 'hidden'; // Hide content briefly
          setTimeout(() => document.body.style.visibility = 'visible', 500);
      }
  });

  // Clear clipboard on Print Screen release
  ...
  // Blur content and show overlay when window loses focus
  window.addEventListener('blur', function() {
      document.querySelector('.content-wrapper').classList.add('unfocused');
      document.body.style.cursor = 'none'; // Hide cursor
  });
  window.addEventListener('focus', function() {
      document.querySelector('.content-wrapper').classList.remove('unfocused');
